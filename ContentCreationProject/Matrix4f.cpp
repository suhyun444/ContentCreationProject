...
Matrix4f Matrix4f::Transpose(const Matrix4f& target)
{
	...
	MathUtil::Swapf(result.Get(1, 1), result.Get(2, 1));
	MathUtil::Swapf(result.Get(1, 2), result.Get(3, 1));
...
Matrix4f Matrix4f::operator*(const Matrix4f other)
{
	...
	for (int i = 0; i < 4; i++)
	{
		result.Get(i, 0) = Get(i, 0) * other.elements[0] + Get(i, 1) * other.elements[1] +
			Get(i, 2) * other.elements[2] + Get(i, 3) * other.elements[3];

		result.Get(i, 1) = Get(i, 0) * other.elements[4] + Get(i, 1) * other.elements[5] +
			Get(i, 2) * other.elements[6] + Get(i, 3) * other.elements[7];

		result.Get(i, 2) = Get(i, 0) * other.elements[8] + Get(i, 1) * other.elements[9] +
			Get(i, 2) * other.elements[10] + Get(i, 3) * other.elements[11];

		result.Get(i, 3) = Get(i, 0) * other.elements[12] + Get(i, 1) * other.elements[13] +
			Get(i, 2) * other.elements[14] + Get(i, 3) * other.elements[15];
	}
